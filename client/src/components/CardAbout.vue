<template>
  <a
    ...
    class="card card-compact w-full md:w-80 lg:w-96 mb-4 bg-neutral hover:bg-accent duration-300 no-underline"
  >
    <figure>
      <img
        :src="department.attributes.coverImage.data.attributes.formats.medium.url"
    ...
    <div class="card-body rounded-b text-left">
      ...
      <ul class="mt-4">
        <li v-for="training in department.attributes.trainings" :key="training.id" class="mt-1">
          {{ training.dayOfTheWeek }}: {{ formatTime(training.start) }} -
          {{ formatTime(training.end) }} at
          <a
            :href="`https://www.google.com/maps?q=${training.location.latitude},${training.location.longitude}`"
            target="_blank"
            rel="noopener noreferrer"
          >
            📍
